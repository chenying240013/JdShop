<%@ page language="java"
	import="java.util.*,com.vo.Jdgood,com.page.PageInfo"
	pageEncoding="UTF-8"%>
<%
...
<body>
	...
	<form id="categoryForm" action="JdgoodServlet?action=getPageByQuery"
		method="post">
	...
	<div class="text-center categoryTableDiv">
				...
				<tr>
					...
					<td>
						...
						<button class="btn btn-success btn-xs"
							onclick="getOneForUpdate(<%=jdgood.getJid()%>)">修改</button>
					</td>
					<td>
						<button class="btn btn-danger btn-xs"
							onclick="deleteCategoryById(<%=jdgood.getJid()%>)">删除</button>
		...
		<div id="pageDiv">
			...
			<form id="categoryForm2"
				action="JdgoodServlet?action=getPageByQuery" method="post">
				<input type="hidden" name="searchCondition"
					value="${requestScope.searchCondition}" /> <a id="first"
					class="btn btn-default btn-sm"
					href="JdgoodServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a>


				<a id="previous" class="btn btn-default btn-sm"
					href="JdgoodServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>">
					上一页</a> <a id="next" class="btn btn-default btn-sm"
					href="JdgoodServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>">
					下一页</a> <a id="last" class="btn btn-default btn-sm"
					href="JdgoodServlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
	...
	<script type="text/javascript">
			...
			function deleteCategoryById(id){
				...
				$.confirm({
					...
					confirm: function() {
						...
						location.assign("JdgoodServlet?action=delete&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&jid=" + id);
					},
					cancel: function() {
			...
			function getOneForUpdate(id){
				...
				location.assign("JdgoodServlet?action=getOneForUpdate&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&jid=" + id);
			}
