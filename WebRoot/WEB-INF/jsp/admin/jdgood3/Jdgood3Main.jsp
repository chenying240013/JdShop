...
<body>
	...
	<form id="categoryForm" action="Jdgood3Servlet?action=getPageByQuery"
		method="post">
	...
	<div class="text-center categoryTableDiv">
		...
		<table id="categoryTable"
			...
			<tbody>
				...
				<%
					List<Jdgood3> list = (List<Jdgood3>) request.getAttribute("list");

					for (Jdgood3 jdgood : list) {
				%>

				<tr>
					<td><%=jdgood.getJ3id()%></td>
					<td><%=jdgood.getJ3name()%></td>
					<td><%=jdgood.getFid()%></td>
					...
					<td>
						...
						<button class="btn btn-success btn-xs"
							onclick="getOneForUpdate(<%=jdgood.getJ3id()%>)">修改</button>
					</td>
					<td>
						<button class="btn btn-danger btn-xs"
							onclick="deleteCategoryById(<%=jdgood.getJ3id()%>)">删除</button>
		...
		<div id="pageDiv">
			...
			<form id="categoryForm2"
				action="Jdgood3Servlet?action=getPageByQuery" method="post">
				<input type="hidden" name="searchCondition"
					value="${requestScope.searchCondition}" /> <a id="first"
					class="btn btn-default btn-sm"
					href="Jdgood3Servlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=1">首页</a>


				<a id="previous" class="btn btn-default btn-sm"
					href="Jdgood3Servlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>">
					上一页</a> <a id="next" class="btn btn-default btn-sm"
					href="Jdgood3Servlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>">
					下一页</a> <a id="last" class="btn btn-default btn-sm"
					href="Jdgood3Servlet?action=getPageByQuery&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>">尾页</a>
	...
	<script type="text/javascript">
			...
			function deleteCategoryById(id){
				...
				$.confirm({
					...
					confirm: function() {
						...
						location.assign("Jdgood3Servlet?action=delete&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&j3id=" + id);
					},
					cancel: function() {
			...
			function getOneForUpdate(id){
				...
				location.assign("Jdgood3Servlet?action=getOneForUpdate&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&j3id=" + id);
			}
