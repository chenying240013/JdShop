<%@ page language="java" import="java.util.*,com.vo.*,com.page.PageInfo,com.service.impl.Jdgood2ServiceImpl" pageEncoding="UTF-8"%>
<%
...
<html>
	...
	<body>
     ...
     %> 
		...
		<form id="categoryForm" action="JdproductServlet?action=getPageByQuery&target=productMainUpload" method="post">
...
<form id="productForm2" action="JdproductServlet?action=getPageByQuery&target=productMainUpload" method="post">

			<div id="pageInfoDiv">
			
	<a id="first" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=1'>首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>'> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>'> 下一页</a>
    
   
     <a id="last" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>'>尾页</a>
	...
	return afterTrimTxt;
			...
			function deleteProductById(pid){
				$.confirm({
				    ...
				    confirm: function(){
				       	//发请求
				       	location.assign("JdproductServlet?action=delete&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
				    },
				    cancel: function(){
			...
			function getOneForUpload(pid){
				...
				location.assign("JdproductServlet?action=getOneForUpload&target=productMainUpload&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
			}
