<%@ page language="java" import="java.util.*,com.vo.*,com.page.PageInfo,com.service.impl.Jdgood2ServiceImpl,com.servlet.*" pageEncoding="UTF-8"%>
<%
...
<html>
	...
	<body>
     ...
     %> 
		...
		<form id="categoryForm" action="JdproductServlet?action=getPageByQuery&target=productMain" method="post">
			...
			<table id="productTable" class="table table-striped table-bordered table-hover">
				...
				%>	
					<tr>
						...
						<td title="<%=product.getPdesc() %>" alt="<%=product.getPdesc() %>"><%=product.getPdesc() %></td>
...
<form id="productForm2" action="JdproductServlet?action=getPageByQuery&target=productMain" method="post">

			<div id="pageInfoDiv">
			
	<a id="first" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=1'>首页</a>
    
    
    <a id="previous" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getPreviousPage()%>'> 上一页</a>
      <a id="next" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getNextPage()%>'> 下一页</a>
    
   
     <a id="last" class="btn btn-default btn-sm" href='JdproductServlet?action=getPageByQuery&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getTotalPageCount()%>'>尾页</a>
	...
	return afterTrimTxt;
			...
			function deleteProductById(pid){
				$.confirm({
				    ...
				    confirm: function(){
				       	//发请求
				       	location.assign("JdproductServlet?action=delete&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
				    },
				    cancel: function(){
			...
			function getOneForUpdate(pid){
				...
				location.assign("JdproductServlet?action=getOneForUpdate&target=productMain&searchCondition=${requestScope.searchCondition}&requestPage=<%=pageInfo.getCurrentPage()%>&pid=" + pid);
			}
